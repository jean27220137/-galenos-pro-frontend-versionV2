...
const FILTROS = [
  { label: 'Todos los despachados', value: 'TODOS' },
  { label: 'Despachados (en tránsito)', value: 'DESPACHADA' },
  { label: 'Entregados y confirmados', value: 'ENTREGADA' },
  { label: 'Rechazados por farmacia',  value: 'RECHAZADA' },
];

@Component({
  selector: 'app-almacen-historial',
  standalone: true,
  ...
  template: `
    <div class="p-6">
      <app-page-header title="Historial de Despachos"
                       subtitle="Solicitudes despachadas y su estado de recepción" />

      <!-- Resumen rápido -->
      <div style="display:flex;gap:1rem;margin-bottom:1rem;flex-wrap:wrap">
        <div *ngFor="let kpi of kpis"
             style="flex:1;min-width:150px;background:#fff;border:1px solid #E2E8F0;
                    border-radius:8px;padding:12px 16px;text-align:center">
          <div [style.color]="kpi.color" style="font-size:1.6rem;font-weight:700">{{ kpi.valor }}</div>
          <div style="font-size:0.78rem;color:#64748B;margin-top:2px">{{ kpi.label }}</div>
        </div>
      </div>
      ...
      <p-table [value]="solicitudesFiltradas"
        ...
        <ng-template pTemplate="header">
          <tr>
            ...
            <th pSortableColumn="fechaSolicitud">Fecha despacho <p-sortIcon field="fechaSolicitud"/></th>
            ...
            <td colspan="7" style="padding:0;background:#F8FAFC;border-top:none">
              <div style="padding:1rem 2rem">
                ...
                <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.75rem">
                  <i class="pi pi-box" style="color:#1A4F8A"></i>
                ...
                <table style="width:100%;border-collapse:collapse;font-size:0.82rem">
                  <thead>
                    <tr style="background:#EFF6FF">
                      ...
                      <th style="padding:6px 10px;text-align:right;color:#1A4F8A;font-weight:600">Cant. Despachada</th>
...
export class AlmacenHistorialComponent implements OnInit {
  ...
  get kpis() {
    return [
      { label: 'Total despachados', valor: this.solicitudes.length, color: '#1A4F8A' },
      { label: 'En tránsito',       valor: this.solicitudes.filter(s => s.estado === 'DESPACHADA').length, color: '#D97706' },
      { label: 'Confirmados',       valor: this.solicitudes.filter(s => s.estado === 'ENTREGADA').length,  color: '#059669' },
      { label: 'Rechazados',        valor: this.solicitudes.filter(s => s.estado === 'RECHAZADA').length,  color: '#DC2626' },
    ];
  }
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      despachadas:  this.solicitudSvc.listarPorEstado('DESPACHADA').pipe(catchError(() => of([]))),
      entregadas:   this.solicitudSvc.listarPorEstado('ENTREGADA').pipe(catchError(() => of([]))),
      rechazadas:   this.solicitudSvc.listarPorEstado('RECHAZADA').pipe(catchError(() => of([]))),
    }).pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: ({ medicamentos, despachadas, entregadas, rechazadas }) => {
          this.medicamentos = medicamentos;
          this.solicitudes  = ([...despachadas, ...entregadas, ...rechazadas] as Solicitud[])
            .sort((a, b) =>
        ...
        error: () => {
          ...
          this.msgSvc.add({ severity: 'error', summary: 'Error',
            detail: 'No se pudo cargar el historial de despachos.', life: 6000 });
          this.cdr.markForCheck();
        }
      });
  }
